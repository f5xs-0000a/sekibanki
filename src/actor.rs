...
use address::Addr;
use context::{Context, ContextImmutHalf};
use message::Message;
...
pub trait Actor: Sized + 'static + Send {
    fn start_actor(self, builder: ActorBuilder, pool: TPSender) -> Addr<Self> {
    ...
    fn on_message_exhaust(&mut self, _ctx: &ContextImmutHalf<Self>) {}
}
